...
namespace Day_6___Vector_Database_Integration_with_Qdrant.Services
{
    public class DocumentChunkingService : IDocumentChunkingService
    {
        ...
        // In DocumentChunkingService.cs, update the ChunkText method:
        ...
        public List<DocumentChunk> ChunkText(string text, string? documentId = null, ChunkingOptions? options = null)
        {
            ...
            {
                // FIXED: Use proper GUID instead of string ID
                chunks[i].Id = DocumentChunk.CreateValidId(documentId, i);
                ...
                if (options.IncludeMetadata)
                {
                    ...
                    chunks[i].Metadata["original_id"] = $"{documentId}_{i}"; // Store original for reference
                }
            }
